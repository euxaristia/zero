...
func reset(s *cursedRenderer) {
	...
	// Hard scroll optimization (using terminal scroll-region sequences to
	// shift existing lines instead of repainting) corrupts rendering badly
	// on terminals that mishandle it. Upstream only disables this on
	// GOOS=="windows", but that only catches a Windows terminal driving the
	// program directly — it misses a Linux process (e.g. inside a multipass
	// VM) whose output is ultimately displayed by Windows Terminal over a
	// remote shell, which hits the same class of bug. There's no reliable
	// way to detect that hop from in here, so disable scroll optimization
	// unconditionally rather than trying to guess.
